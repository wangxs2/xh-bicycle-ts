...
export default class orderSituation extends Vue {
  // 工单数据分组
  private workOrder: Array<any> = []
  // 当前显示的工单数据
  private WorkOrderItem: Array<any> = []
  // 全部的echarts对象
  private echartNodes: Array<any> = []
  // 工单数据定时器
  ...
  private dispatchOrder(data: any): void {
    // 年月日 保留月日
    const timeArr: Array<string> = data.timeArea.map(
      (item: any): void => {
        return item.slice(5)
      }
    )
    ...
    // 处理数据
    let WorkOrder: Array<any> = data.data.map(
      (item: any): any => {
  ...
  private orderSlideShow(index: number): void {
    ...
    let WorkOrder: Array<any> = this.workOrder
