...
export default class WorkDispose extends Vue {
  // 处置率数据
  private roughHandling: any[] = [];
  // 当前显示的处置率数据
  private roughHandlingItem: any[] = [];
  // 工单数据定时器
  ...
  private orderSlideShow(index: number): void {
    ...
    const WorkOrder: any[] = this.roughHandling;
    ...
    for (let i: number = 0; i < 3; i++) {
      if (WorkOrder[index][i]) {
        ...
        MyEchart.clearContainer(`disContainer${i + 1}`);
      }
    }
